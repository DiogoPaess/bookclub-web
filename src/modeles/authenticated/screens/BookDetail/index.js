...
export const BookDetailScreen = () => {
  ...
  const addFavoriteMutation = useMutation((data) => addBookToFavorites(data), {
    onError: (error) => {
      toast({
        title: 'Error ao adicionar livros aos favoritos.',
        description:
  ...
  const deleteFavoriteMutation = useMutation(
    (data) => deleteBookFromFavorites(data),
    {
      onError: (error) => {
        toast({
          title: 'Error ao remover livro dos favoritos.',
          description:
            error?.response?.data?.error || 'Por favor tente novamente.',
          status: 'error',
          duration: 3000,
      ...
      onSuccess: () => {
        toast({
          title: 'Livros removido dos favoritos com sucesso!',
          status: 'success',
          duration: 6000,
          isClosable: true
        })
    ...
    <Flex flexDir="column">
      ...
      >
        ...
        >
          ...
          <Text maxW="80%" mt="4px" fontSize="10px" color="brand.greyDark">
